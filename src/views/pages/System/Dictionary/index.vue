  ...
  <div class="wh-full p-4 flex">
    <section class="dict-category-wrapper wrapper-item w-96 h-full flex flex-col">
      ...
      <el-collapse v-model="currentCategory" class="overflow-auto flex-1 h-0">
        <el-collapse-item v-for="(item, index) in categoryList" :key="index" :title="item.label" :name="item.code">
          <template #title>
            <span class="text-sm ml-4 font-beauty">{{ item.label }}（{{ item.code }}）</span>
          </template>
          <ul class="px-4">
            <li v-for="subItem in item.children" :key="subItem.code" class="py-2 px-4 hover-item hover-common-wrapper"
    ...
    <section class="dict-item-wrapper wrapper-item flex-1 w-0 h-full ml-4 flex flex-col">
    ...
    <div v-if="formDialogProps.visible">
      <DictDialog :visible="formDialogProps.visible" :opt-type="formDialogProps.optType" :row="formDialogProps.row"
        :current-code="currentDict" @close="closeDialog" @change-success="getDictList"></DictDialog>
    </div>
  </div>
</template>
