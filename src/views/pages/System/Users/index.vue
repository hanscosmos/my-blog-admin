<template>
  <div class="wh-full">
      ...
      <template #header>
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <app-tag size="large" class="mr-4">用户昵称</app-tag>
            ...
            <app-tag size="large" class="mr-4">用户角色</app-tag>
            <el-select v-model="searchParams.roleId" clearable filterable class="!w-280px"
        ...
        <el-table :data="dataList" style="width: 100%" row-key="id" size="large" border stripe default-expand-all>
          <el-table-column label="基本信息" align="center">
            <template #default="{ row }">
              <div class="xy-center">
                <img v-if="row.avatar" :src="row.avatar" class="w-8 h-8 mr-4 rounded-full" />
          ...
          <el-table-column label="角色" align="center">
            <template #default="{ row }">
              <div class="xy-center gap-2">
                <AppTag color="#999" v-for="item in row.roles" :key="item.id" :name="item.name"></AppTag>
              </div>
            </template>
          </el-table-column>
    ...
    <div v-if="formDialogProps.visible">
      <AddUserDialog :opt-type="formDialogProps.optType" :visible="formDialogProps.visible" :row="formDialogProps.row"
        :role-list="roleList" @close="closeDialog" @change-success="filterDataListHandler"></AddUserDialog>
    </div>
  </div>
</template>
