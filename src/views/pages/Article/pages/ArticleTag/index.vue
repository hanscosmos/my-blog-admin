<template>
    ...
    <AppSearchPanel :data-exist="dataList.length > 0" :loading="loading">
      <template #header>
        <div class="flex items-center">
          ...
          <app-button v-perm="'article:tag:add'" @click="openDialog('add')">
            <AppIcon name="add" class="mr-2"></AppIcon>新增标签
          </app-button>
        </div>
      </template>
      <div class="p-4">
        <el-table :data="dataList" style="width: 100%" row-key="id" size="large" border stripe default-expand-all>
          ...
          <el-table-column prop="color" align="center" label="颜色">
            <template #default="{ row }">
              <AppTag :name="getValueByKey(colorList, row.color) as string" :color="row.color"></AppTag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" fixed="right" width="250" align="center">
            <template #default="{ row }">
              <div flex w-full class="justify-center">
                <el-button v-perm="'article:tag:update'" link type="primary" @click="openDialog('edit', row)">
                  编辑
                </el-button>
                <el-button v-perm="'article:tag:delete'" link type="danger" plain> 删除 </el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
  ...
  <div v-if="formDialogProps.visible">
    <TagDialog :visible="formDialogProps.visible" :opt-type="formDialogProps.optType" :row="formDialogProps.row"
      :color-list="colorList" @change-success="filterDataListHandler" @close="closeDialog"></TagDialog>
  </div>
</template>
