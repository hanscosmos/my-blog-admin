<template>
  <div class="wh-full">
    <AppSearchPanel :data-exist="dataList.length > 0" :loading="loading">
      <template #header>
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <app-tag size="large" class="mr-4">文章标题</app-tag>
            <el-input v-model="searchParams.title" class="!w-280px mr-4" placeholder="请输入关键词搜索" clearable
              @change="filterDataListHandler"></el-input>
            <app-tag size="large" class="mr-4">文章类别</app-tag>
            <el-tree-select v-model="searchParams.category" :data="categoryList" node-key="id" clearable check-strictly
              filterable :props="{ label: 'name' }" class="!w-280px" @change="changeCategory" />
          </div>
      ...
      <div class="p-4">
        <el-table v-if="currentLayout === 'list'" :data="articleList" style="width: 100%" row-key="id" size="large"
          ...
          <el-table-column label="作者" align="center">
            <template #default="{ row }">
              <div class="xy-center">
                <img v-if="row.authorAvatar" :src="row.authorAvatar" class="w-8 h-8 rounded-full mr-4" />
                <span class="">
                  {{ row.author }}
                </span>
              </div>
