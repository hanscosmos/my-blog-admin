<template>
  <el-drawer :model-value="visible" :size="500" :lock-scroll="false" :before-close="closeDrawer" title="发布文章设置">
    <el-form label-width="100px" ref="formRef" :rules="articleFormRules" :model="articleForm">
      <el-form-item prop="category" label="文章分类">
        <el-tree-select v-model="articleForm.category" :data="categoryList" node-key="id" :props="{ label: 'name' }"
          class="!w-full" />
      </el-form-item>
      ...
      <el-form-item prop="column" label="文章专栏">
        <el-select v-model="articleForm.column as string">
          <el-option v-for="item in columnList" :key="item.id" :value="item.id" :label="item.name">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item prop="original" label="是否原创">
        <el-radio-group v-model="articleForm.properties">
          <el-radio v-for="item in propertiesList" :key="item.key" :value="item.key">
            {{ item.value }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item prop="visible" label="文章可见性">
        <el-radio-group v-model="articleForm.visible">
          <el-radio v-for="item in visibleList" :key="item.key" :value="item.key">
            {{ item.value }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item prop="cover" label="文章封面">
        <div class="w-full h-48">
          <AppImageAutoUpload type="article-cover" desc="上传文章封面" editable :default-url="articleForm.cover"
            @upload-success="uploadSuccessHandler"></AppImageAutoUpload>
        </div>
      </el-form-item>
      <el-form-item prop="abstract" label="编辑摘要">
        <el-input v-model="articleForm.abstract" type="textarea" :rows="4" maxlength="100" show-word-limit
          resize="none"></el-input>
      </el-form-item>
    </el-form>
    <div class="flex justify-end mt-4">
      <app-button type="plain" class="mr-4" @click="closeDrawer">取消</app-button>
      <app-button @click="publishArticleHandler" class="mr-4">发布文章</app-button>
      <el-button v-if="!defaultData?.status || defaultData?.status !== statusList[1].key" type="primary"
        @click="saveDraftsHandler">保存草稿</el-button>
    </div>
  </el-drawer>
</template>
