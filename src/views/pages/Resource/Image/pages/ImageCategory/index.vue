    ...
    <el-table :data="categoryList" height="100%" size="large" border stripe>
      ...
      <el-table-column fixed="right" width="200" align="center">
        <template #header>
          <div class="xy-center">
            <span>操作</span>
            <AppIcon v-perm="'resource:image-category:add'" name="plus" class="hover-text ml-2"
              @click="openDialog('add')"></AppIcon>
          </div>
        </template>
        <template #default="{ row }">
          <div flex w-full class="justify-center">
            <el-button v-perm="'resource:image-category:update'" link type="primary" @click="openDialog('edit', row)">
              修改
            </el-button>
            <el-button v-perm="'resource:image-category:delete'" link type="danger"> 删除 </el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
    <div v-if="formDialogProps.visible">
      <ImageCategoryDialog :visible="formDialogProps.visible" :row="formDialogProps.row"
        :opt-type="formDialogProps.optType" @close="closeDialog" @change-success="getCategoryList">
      </ImageCategoryDialog>
    </div>
  </div>
</template>
