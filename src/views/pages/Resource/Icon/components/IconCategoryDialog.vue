...
import { addIconCategoryApi, editIconCategoryApi } from '@/api/resource/icon';
...
const dialogTitle = computed(() => {
  return props.optType === 'add' ? '创建图标类别' : '编辑图标类别';
});
  ...
  formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        const { data, msg } =
          props.optType === 'add'
            ? await addIconCategoryApi(form.value)
            : await editIconCategoryApi({ id: props.row.id, ...form.value });
        if (data) {
