<template>
  <div class="flex flex-col wh-full" :class="currentLayout">
    <div class="header-wrapper border-bottom p-4 flex justify-between items-center">
    ...
    <div class="category-wrapper border-bottom h-10">
      <ul class="flex items-center h-full px-4">
        <li class="hover-wrapper text-xs rounded px-2 py-1 mr-2"
          :class="{ 'active-item': searchParams.category === item.id }" v-for="item in categoryList" :key="item.id"
    ...
    <div v-loading="loading" class="icon-wrapper flex-1 h-0 flex flex-col p-4">
      <ul class="icon-list-wrapper flex-1 h-0 overflow-auto flex flex-wrap gap-3">
    ...
    <div v-if="formDialogProps.visible">
      <IconFormDialog :visible="formDialogProps.visible" :opt-type="formDialogProps.optType" :row="formDialogProps.row"
